...
function EditEvent() {
  ...
  const successToast = () => {
    toast.success(<p className="m-2">You've successfully edited an event!</p>, {
  ...
  const handleEditEvent = async (event) => {
    ...
    // Append cover image: check if it's a file or existing URL
    if (coverImage instanceof File) {
      formData.append('coverImage', coverImage);
    } else if (coverImage) {
      formData.append('existingCoverImage', coverImage);  // Append existing image URL if no new image is uploaded
    }

    // Append additional images: check if each image is a file or an existing URL
    additionalImages.forEach((file, index) => {
      if (file instanceof File) {
        formData.append(`additionalImage${index + 1}`, file);
      } else if (file) {
        formData.append(`existingAdditionalImage${index + 1}`, file);  // Append existing image URL if no new image is uploaded
      }
    });
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      ...
      setCoverImage(eventData.coverImage ? `http://localhost:5000/images/${eventData.coverImage}` : null);
      setAdditionalImages([
        eventData.additionalImage1 ? `http://localhost:5000/images/${eventData.additionalImage1}` : null,
        eventData.additionalImage2 ? `http://localhost:5000/images/${eventData.additionalImage2}` : null,
        eventData.additionalImage3 ? `http://localhost:5000/images/${eventData.additionalImage3}` : null
      ]);
      setSelectedSubCategory(eventData.subCategory || '');
      setIsFreeTicket(eventData.ticketPrice === 0);
      setTicketPrice(eventData.ticketPrice || '');
      document.querySelector('input[name="eventName"]').value = eventData.name || '';
      document.querySelector('textarea[name="eventDescription"]').value = eventData.description || '';
      document.querySelector('select[name="eventCountry"]').value = eventData.country || '';
      document.querySelector('input[name="eventDate"]').value = formattedDate || '';
      document.querySelector('input[name="eventStartTime"]').value = eventData.startTime || '';
      document.querySelector('input[name="eventEndTime"]').value = eventData.endTime || '';
      document.querySelector('input[name="venueName"]').value = eventData.venueName || '';
      document.querySelector('input[name="venueAddress"]').value = eventData.venueAddress || '';
      document.querySelector('input[name="capacity"]').value = eventData.capacity || '';
    };
  ...
  return (
    <>
      ...
      <div className="container auth-wrapper d-flex justify-content-center" style={{ maxWidth: '100%' }}>
        <Form onSubmit={handleEditEvent} className="w-100">
          <div className="d-flex flex-wrap justify-content-around">
              ...
              <Form.Group>
                ...
                <div
                  ...
                  style={{
                    ...
                    padding: '50px',
                    textAlign: 'center',
                    backgroundColor: coverImage ? 'transparent' : 'lightgrey',
                ...
                <div className='d-flex flex-column align-items-center'>
                  <div className='d-flex justify-content-around' style={{ width: "100%" }}>
                    {[0, 1].map((index) => (
                      <div
                        ...
                        style={{
                          backgroundImage: additionalImages[index] ? `url(${additionalImages[index]})` : 'none',
                        ...
                        onClick={() => document.getElementById(`additional-image-input-${index}`).click()}
                      >
                        <input
                  ...
                  <div className="mt-3" style={{ width: "100%" }}>
                    <div
                      className="upload-zone upload-zone-small"
                      style={{
                        backgroundImage: additionalImages[2] ? `url(${additionalImages[2]})` : 'none',
                      ...
                      onClick={() => document.getElementById('additional-image-input-2').click()}
                    >
                      <input
            ...
            <div className="px-5 py-3 container" style={{ width: '50%', backgroundColor: "white" }}>
              ...
              <Button style={{ height: "50px", borderRadius: "20px", fontFamily: "BudujSansRegular" }} variant="primary" type="submit" className="w-100">Edit Event</Button>
            </div>
          </div>
        </Form>
      </div>
